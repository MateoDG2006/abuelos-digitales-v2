...
export function Login({ onLogin, onBack }: LoginProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.phone && formData.cedula) {
      localStorage.setItem('user', JSON.stringify({ ...formData, role: 'volunteer' }));
      initializeMockData();
      onLogin();
    }
  };
  ...
  const handleOAuth = (provider: string) => {
    localStorage.setItem('user', JSON.stringify({ fullName: 'Usuario Demo', role: 'volunteer' }));
    initializeMockData();
    onLogin();
  };
  ...
  return (
    <div className="bg-white rounded-3xl p-8 shadow-lg">
      ...
      <div className="text-center mb-6">
        ...
        <p className="text-slate-600">Ingresa con tu teléfono y cédula</p>
      </div>
